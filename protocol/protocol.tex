...
\title{Investigating the Causal Effect of Stroke and Traumatic Brain Injury on Healthcare Access and Utilization in the U.S. Adult Population: A Propensity Score--Matched Analysis of NHANES 2011--2014}
...
Investigating the Causal Effect of Stroke and Traumatic Brain Injury on Healthcare Access and Utilization in the U.S. Adult Population: A Propensity Score--Matched Analysis of NHANES 2011--2014
...
\textbf{Objectives:} This study will estimate the causal effect of self-reported lifetime history of two exposures: (1) stroke and (2) traumatic brain injury (TBI) with loss of consciousness, on several outcomes (i.e., multiple indicators of healthcare access and utilization) in a nationally representative sample of U.S. adults.
...
\textbf{Results and Conclusions:} This document describes the protocol and planned analyses. After the publication of this protocol, the required analyses will be completed and numerical results and final conclusions will be detailed in a separate manuscript.
...
\begin{enumerate}
    ...
    \item Evaluate whether these effects vary across demographic and socioeconomic subgroups (e.g., age, sex, race/ethnicity, education, income).

    \item Implement propensity score methods combined with NHANES survey weights, following \citet{dugoff2014}, to obtain effect estimates that both control for observed confounding and generalize to the national target population.
...
Among noninstitutionalized U.S. adults represented by NHANES 2011--2014, what is the causal effect of self-reported lifetime history of:
...
on multiple indicators of healthcare access and utilization, including having a usual source of care and health insurance coverage?
...
In addition to the primary hypotheses above, we will conduct planned exploratory analyses to
assess whether the effects of (1) TBI history on healthcare access/utilization and
(2) stroke history on healthcare access/utilization vary across key demographic and
socioeconomic subgroups. These analyses are intended to characterize heterogeneity in
associations and generate hypotheses for future work rather than to provide definitive
subgroup-specific inference.
We will evaluate effect modification by age, sex, race/ethnicity, education, and income-to-poverty ratio
...
\begin{enumerate}
    ...
    \item Estimate the population-wide average treatment effect on the treated (PATT) between stroke history and each healthcare access/utilization outcome, adjusting for observed confounders using propensity score matching combined with survey design-based analysis.

    \item Estimate the analogous causal effect for TBI.
...
This is an observational, cross-sectional study using existing NHANES data to estimate the causal effect of lifetime neurologic injury (stroke, TBI) on healthcare access/utilization. The principal design elements are:
...
We will treat age, sex, race/ethnicity, education, and income as candidate effect modifiers and will explore effect modification in secondary analyses.
...
\begin{itemize}
    \item Achieving SMD $< 0.10$ for all prognostically relevant covariates,
...
For stroke, the 1:4 matched sample met the balance criterion across all covariates (max $|\text{SMD}| = 0.091$; 1 covariate with $|\text{SMD}| > 0.05$), retained all 431 treated participants, and yielded a total matched sample size of 2,155 for outcome modeling. For TBI, the 1:4 matched sample also met the balance criterion (max $|\text{SMD}| = 0.093$; 4 covariates with $|\text{SMD}| > 0.05$), retained all 948 treated participants, and yielded a total matched sample size of 4,740. All outcome analyses (Section~7) will be conducted using these 1:4 matched datasets.

\paragraph{Love plots (1:4 matching).}
Figure~\ref{fig:love-stroke} and Figure~\ref{fig:love-tbi} display standardized mean differences (SMD) for each covariate before and after 1:4 propensity score matching for the stroke and TBI analyses, respectively. Values closer to zero indicate improved balance after matching. The vertical reference lines at $\pm0.10$ denote the acceptable balance threshold. Love plots (and balance tables) for all evaluated ratios (1:1--1:6) will be summarized in the final manuscript and/or supplement.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../matching/love_stroke_1_4.png}
\caption{Covariate balance before and after 1:4 propensity score matching: stroke vs.\ no stroke. The vertical lines at $\pm0.10$ indicate the acceptable balance threshold.}
...
\caption{Covariate balance before and after 1:4 propensity score matching: TBI vs.\ no TBI. The vertical lines at $\pm0.10$ indicate the acceptable balance threshold.}
\label{fig:love-tbi}
\end{figure}

Diagnostic plots and summary statistics will be included in the final manuscript to empirically demonstrate balance in the matched samples.
...
\paragraph{Results.}
Table~\ref{tab:neg-control} presents the negative control diagnostic results for the 1:4 matched samples. For stroke, the SMD for marijuana ever-use was $<0.10$ and the odds ratio 95\% CI included 1, indicating successful balance on the negative control. For TBI, the SMD exceeded 0.10 and the odds ratio 95\% CI excluded 1, suggesting residual imbalance on constructs correlated with marijuana ever-use; per the prespecified response, we will revise the design of the TBI analysis to include marijuana ever-use.

As before, the 1:4 matched sample met the balance criterion (max $|\text{SMD}| = 0.096$; 5 covariates with $|\text{SMD}| > 0.05$), retained all 948 treated participants, and yielded a total matched sample size of 4,740. A revised love plot is included in Figure \ref{fig:love-tbi-revised}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../matching/love_tbi_1_4_revised.png}
\caption{Covariate balance before and after 1:4 propensity score matching (including marijuana ever-use): TBI vs.\ no TBI. The vertical lines at $\pm0.10$ indicate the acceptable balance threshold.}
\label{fig:love-tbi-revised}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Negative control balance check: marijuana ever-use in matched samples (1:4 matching)}
\label{tab:neg-control}
\begin{tabular}{lcccccc}
\hline
\textbf{Dataset} & \textbf{$n$} & \textbf{Exposed} & \textbf{Unexposed} & \textbf{SMD} & \textbf{OR (95\% CI)} & \textbf{Balance OK?} \\
\hline
Stroke & 429 & 91 & 338 & $-0.05$ & 0.90 (0.53, 1.52) & Yes \\
TBI & 2,217 & 460 & 1,757 & 0.16 & 1.41 (1.16, 1.72) & No \\
\hline
\end{tabular}
\end{table}
...
Within each matched sample (stroke vs.\ no stroke; TBI vs.\ no TBI), we estimate the causal effect of exposure status on each healthcare access/utilization outcome using interview-weighted regression models.
